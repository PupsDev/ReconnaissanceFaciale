...
def getEyes(frame):
    ...
    if (eye_1[0] is not None) and (eye_2[0] is not None):
        ...
        xtranslate = frame.shape[1]/2 - (right_eye_center[0]+left_eye_center[0])/2
        ytranslate = frame.shape[0]/2 - (right_eye_center[1]+left_eye_center[1])/2
...
def align(frame):
    ...
    if (eye_1[0] is not None) and (eye_2[0] is not None):
        ...
        new_img = new_img.transform(new_img.size, Image.AFFINE, (1, 0, -xtranslate, 0, 1, -ytranslate))
...
def save(frame,i,j, name):
    ...
    cv2.imwrite("ressource/dataset/treated/database2/face"+format(i, '02d')+'_'+format(j, '02d')+name+".jpg", frame)

def save1(frame,i, name):
    frame = cv2.resize(frame, (64,64), interpolation = cv2.INTER_AREA)
    cv2.imwrite("ressource/dataset/treated/database2/face"+format(i//6+1, '02d')+'_'+format(i%6, '02d')+name+".jpg", frame)
def open(set,i ,j):

        frame = cv2.imread('ressource/dataset/untreated/dataset'+str(set)+'/face'+format(i, '02d')+"_"+format(j, '02d')+'.jpg')
...
def open1(set,i):
        ...
        frame = cv2.imread('ressource/dataset/untreated/dataset/'+format(i, '04d')+'.jpg')
